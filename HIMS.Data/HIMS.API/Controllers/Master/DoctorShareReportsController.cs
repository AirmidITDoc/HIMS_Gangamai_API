...
namespace HIMS.API.Controllers.Transaction
{
    ...
    public class DoctorShareReportsController : Controller
    {
        ...
        public IActionResult ViewDoctorShareReport(int Doctor_Id, int GroupId, DateTime From_Dt, DateTime To_Dt, int OP_IP_Type)
        {
            ...
            var tuple = _pdfUtility.GeneratePdfFromHtml(html, "DoctorShareDetailReport", "DoctorShareDetailReport", Wkhtmltopdf.NetCore.Options.Orientation.Landscape);


            return Ok(new { base64 = Convert.ToBase64String(tuple.Item1) });
        }
        [HttpGet("viewDoctorWiseSummaryReport")]
        public IActionResult ViewDoctorWiseSummaryReport(int Doctor_Id, int GroupId, DateTime From_Dt, DateTime To_Dt, int OP_IP_Type)
        {
            string htmlFilePath = Path.Combine(_hostingEnvironment.WebRootPath, "PdfTemplates", "DoctorShare_DoctorWiseSummaryReport.html");
            string htmlHeaderFilePath = Path.Combine(_hostingEnvironment.WebRootPath, "PdfTemplates", "NewHeader.html");
            var html = _doctorShareReport.ViewDoctorWiseSummaryReport( Doctor_Id,  GroupId,  From_Dt,  To_Dt, OP_IP_Type, htmlFilePath, _pdfUtility.GetHeader(htmlHeaderFilePath));
